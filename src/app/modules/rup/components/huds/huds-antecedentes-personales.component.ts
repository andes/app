...
@Component({
    selector: 'huds-antecedentes-personales',
    templateUrl: 'huds-antecedentes-personales.html',
    styleUrls: []
})

export class HudsAntecedentesPersonalesComponent implements OnChanges {
    ...
    get antecedentesFiltrados() {
        ...
        if (!searchTerm) {
            return this.antecedentesP;
        }
        const tokens = searchTerm.split(' ');
        return this.antecedentesP.filter(antecedente => {
    ...
    cargarAntecedentesPersonales() {
        ...
        this.hudsService.getAntecedentesPersonales(this.paciente.id).subscribe(antecedentes => {
            this.antecedentesP = antecedentes;
        });
    }

    toggleAcordeon(antecedente, active) {
        this.antecedenteExpandido = active ? antecedente : null;
    }
