...
export class Auditoria2Component implements OnInit {
    ...
    match = new matching();
    // Crear una entrada en el config con estos pesos para auditoria
    weights = {
                identity: 0.3,
                name: 0.3,
                gender: 0.1,
    ...
    inicializar() {
        ...
        this.pacienteService.getById(this.destiny.id).subscribe(data => {
            if (data) {
                ...
                this.loadPacientesVinculados(data.identificadores);
                if (this.origin) {
     ...
     */
    loadPacientesVinculados(idsPacientesVinculados) {
    ...
    onBack() {
        this.data.emit(null);
        // vaciamos los arrays
        this.pacientesVinculados = [];
        this.pacientesDesvinculados = [];
    }
}
