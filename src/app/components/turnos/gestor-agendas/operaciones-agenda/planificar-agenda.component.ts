...
export class PlanificarAgendaComponent implements OnInit {
    ...
    onSave($event, clonar: Boolean) {
        ...
        const observables$ = this.modelo.tipoPrestaciones.map(prestacion =>
            this.conceptoTurneablesService.search({ ids: prestacion._id }).pipe(
                catchError(() => of([]))
            )
        );
        ...
        forkJoin(observables$).pipe(
            ...
            next: (resultado) => {
                this.manejarExito(resultado, clonar);
            },
            error: () => {
    ...
    manejarErrores(incluyeAmbulatorio, prestacionesText, bloqueConPrestActiva, arrayPrestaciones) {
        if (!this.verificarNoNominalizada()) {
            ...
            this.plex.info('warning', `Las prestaciones <b>${prestacionesText}</b> ya no están habilitadas para crear agendas.`);
        } else {
