  ...
  */
  ...
  onReturnComponent(datos, tipoPrestacionActual) {
    ...
    */
    ...
    /*
    let posicionPrestacion = this.prestacionesEjecucion.findIndex( prestacion => (prestacion.solicitud.tipoPrestacion.id === tipoPrestacionActual.id) );
    console.log(posicionPrestacion);
    if (posicionPrestacion >= 0) {
      this.prestacionesEjecucion[posicionPrestacion].ejecucion.evoluciones.push({ valores: { [tipoPrestacionActual.key]: this.data[tipoPrestacionActual.key] } });

      this.updateGeneral = true;
      //this.updateGeneral = false;
    }
    */
  ...
  onPrestacionDrop(e: any, idProblema) {
    ...
    if (posicionPrestacion < 0) {
      ...
      // asignamos la evolucion a la prestacion en caso 
      // que se haya cargado algun valor
      /*
      if (this.data[e.dragData.key]) {
        nuevaPrestacion.ejecucion.evoluciones.push({ valores: { [e.dragData.key]: this.data[e.dragData.key] } });
      }
      */
