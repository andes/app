...
var BarrioService = (function () {
    function BarrioService(http) {
        this.http = http;
        this.barrioUrl = 'http://localhost:3002/api/barrio'; // URL to web api
    }
    BarrioService.prototype.get = function () {
        return this.http.get(this.barrioUrl)
            .map(function (res) { return res.json(); })
            .catch(this.handleError); //...errors if any*/
    };
    BarrioService.prototype.getXProvincia = function (localidad) {
        console.log(this.barrioUrl + "?localidad=" + localidad);
        return this.http.get(this.barrioUrl + "?localidad=" + localidad)
            .map(function (res) { return res.json(); })
            .catch(this.handleError); //...errors if any*/
    };
    BarrioService.prototype.handleError = function (error) {
