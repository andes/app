                ...
                return this.pacienteHttp.match({
                    ...
                    map(resultadoSuggest => {
                        ...
                        if (!resultadoSuggest.length) {
                            return { pacientes: [pacienteEscaneado], escaneado: true, scan: textoLibre, err: null };
                        }
                        // 1.3.2. Busca a uno con el mismo código de barras
                        let candidato = resultadoSuggest.find(i => i.paciente.scan && i.paciente.scan === textoLibre);
                        if (candidato) {
