...
export class ArbolPermisosItemComponent implements OnInit, OnChanges {
    ...
    checkIncompatibles($event) {
        ...
        if ($event.value) {
            ...
            const permisoIncompatible = this.permisosIncompatibles.find(par => par.includes(permisoActual))?.filter(str => str !== permisoActual)[0];
            if (permisoIncompatible) {
                // si existen permisos incompatibles con el actual, se desactiva el que estaba previamente activo
